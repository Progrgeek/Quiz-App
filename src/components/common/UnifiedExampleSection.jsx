...
/**
 * Unified Example Section Component
 * Uses data from unified JSON structure instead of hardcoded content
 */
const UnifiedExampleSection = ({ children, data }) => {
  const [mode, setMode] = useState("practice");
  const sectionRef = useRef(null);
  
  // Extract UI text from data instead of hardcoding
  const uiText = data.ui || {};
    ...
    >
      <div className="bg-white p-6 sm:mt-14 rounded-lg border-2 border-gray-200 mb-8">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          {exampleContent.practiceExample?.question || "Example Exercise"}
        </h2>
        <p className="text-gray-600 mb-6">
          {exampleContent.description || "Here's how this exercise works."}
        </p>

        {/* Dynamic categories from JSON */}
        {exampleContent.practiceExample?.categories && (
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {Object.entries(exampleContent.practiceExample.categories).map(([category, items]) => (
              <div
                key={category}
        ...
        {/* Dynamic learning points from JSON */}
        {exampleContent.learningPoints && (
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="font-medium text-gray-800 mb-2">
              {uiText.labels?.learningPoints || "Learning Points:"}
            </h3>
      ...
      {/* Dynamic introduction and demonstrations from JSON */}
      {exampleContent.introduction && (
        <div className="bg-white p-6 rounded-lg border-2 border-gray-200 mb-8">
          ...
          {/* Dynamic demonstrations table */}
          {exampleContent.demonstrations && Object.entries(exampleContent.demonstrations).map(([sense, details]) => (
                  ...
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="px-2 sm:px-4 py-2 text-left border border-gray-200 w-1/2">
                        {uiText.labels?.example || "Example"}
                      </th>
                      <th className="px-2 sm:px-4 py-2 text-left border border-gray-200 w-1/2">
                        {uiText.labels?.explanation || "Explanation"}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
  ...
  return (
    <div ref={sectionRef} className="w-full max-w-5xl mx-auto">
      ...
      >
        {mode === "practice" 
          ? (uiText.buttons?.learnWithExample || "Learn with examples")
          : (uiText.buttons?.backToPractice || "Back to practice")
        }
        {mode === "practice" ? (
...
UnifiedExampleSection.propTypes = {
  children: PropTypes.node.isRequired,
  data: PropTypes.shape({
    ui: PropTypes.object,
    exampleContent: PropTypes.object,
  }).isRequired,
};

export default UnifiedExampleSection;
